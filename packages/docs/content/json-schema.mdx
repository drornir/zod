...
These schemas are supported via standard base64 JSON Schema metadata:

```ts
z.base64(); // => { type: "string", format: "base64", contentEncoding: "base64" }
z.base64JS(); // => { type: "string", format: "base64", contentEncoding: "base64" }
```
...
For `z.base64()`, `z.base64url()`, and `z.base64JS()`, the emitted `pattern` changes to match the selected padding and last-chunk options. Standard JSON Schema metadata is only emitted for the standard base64 encoding.
